          ...
          <el-card class="main-form-card" shadow="hover">
            <template #header>
              <div class="card-header">
                <h3>{{ isHoldingAnalysisMode ? '持仓配置' : '分析配置' }}</h3>
                <el-tag type="info" size="small">{{ isHoldingAnalysisMode ? '持仓必填' : '必填信息' }}</el-tag>
              </div>
            </template>
            ...
            <el-form :model="analysisForm" label-width="100px" class="analysis-form">
              ...
              <div class="form-section">
                <div class="section-title-row">
                  ...
                  <el-switch
                    v-model="analysisForm.holding.enabled"
                    active-text="纳入分析"
                    inactive-text="不纳入"
                  />
                </div>

                <div v-if="analysisForm.holding.enabled" class="holding-form">
                  <el-row :gutter="16">
                    <el-col :xs="24" :sm="12">
                      <el-form-item label="成本价" required>
                        <el-input-number
                          v-model="analysisForm.holding.costPrice"
                          :min="0"
                          :precision="3"
                          :step="0.1"
                          size="large"
                    ...
                    <el-col :xs="24" :sm="12">
                      <el-form-item label="持股数量" required>
                        <el-input-number
                          v-model="analysisForm.holding.shares"
                          :min="0"
                          :step="100"
                          size="large"
                          controls-position="right"
                    ...
                    <el-col :xs="24" :sm="12">
                      <el-form-item label="止盈价">
                        <el-input-number
                          v-model="analysisForm.holding.takeProfitPrice"
                          :min="0"
                          :precision="3"
                          :step="0.1"
                          size="large"
                    ...
                    <el-col :xs="24" :sm="12">
                      <el-form-item label="止损价">
                        <el-input-number
                          v-model="analysisForm.holding.stopLossPrice"
                          :min="0"
                          :precision="3"
                          :step="0.1"
                          size="large"
                    ...
                    <el-col :xs="24" :sm="12">
                      <el-form-item label="持仓周期">
                        <el-select
                          v-model="analysisForm.holding.plan"
                          size="large"
                          style="width: 100%"
                        >
          ...
          <el-col :span="24">
            <el-card class="results-card" shadow="hover">
              ...
              <div class="results-content">
                ...
                <!-- 持仓分析 -->
                <div v-if="analysisResults.holding_analysis" class="holding-analysis-section">
                  <h4>持仓分析</h4>
                  <div class="holding-analysis-card">
                    <div class="holding-analysis-head">
                      <div>
                        <div class="holding-status">{{ analysisResults.holding_analysis.status }}</div>
                        <div class="holding-subtitle">参考卖出比例 {{ analysisResults.holding_analysis.sell_ratio_text }}</div>
                      </div>
                      <el-tag :type="getHoldingRiskTagType(analysisResults.holding_analysis.risk_level)" size="large">
                        {{ analysisResults.holding_analysis.risk_level }}风险
                      </el-tag>
                    </div>

                    <div class="holding-metrics">
                      <div class="holding-metric">
                        <span class="metric-label">参考卖出数量</span>
                        <strong>{{ analysisResults.holding_analysis.sell_shares_text }}</strong>
                      </div>
                      <div class="holding-metric">
                        <span class="metric-label">持仓盈亏</span>
                        <strong :class="getProfitClass(analysisResults.holding_analysis.profit_loss)">
                          {{ formatMoney(analysisResults.holding_analysis.profit_loss) }}
                          <span v-if="analysisResults.holding_analysis.profit_loss_pct !== null">
                            ({{ analysisResults.holding_analysis.profit_loss_pct }}%)
                          </span>
                        </strong>
                      </div>
                      <div class="holding-metric">
                        <span class="metric-label">当前参考价</span>
                        <strong>{{ formatPrice(analysisResults.holding_analysis.current_price) }}</strong>
                      </div>
                      <div class="holding-metric">
                        <span class="metric-label">距止盈</span>
                        <strong>{{ formatPct(analysisResults.holding_analysis.distance_to_take_profit_pct) }}</strong>
                      </div>
                      <div class="holding-metric">
                        <span class="metric-label">距止损</span>
                        <strong>{{ formatPct(analysisResults.holding_analysis.distance_to_stop_loss_pct) }}</strong>
                      </div>
                    </div>

                    <div class="holding-suggestion">
                      {{ analysisResults.holding_analysis.suggestion }}
                    </div>
                    <el-alert
                      type="info"
                      :closable="false"
                      class="holding-notice"
                    >
                      <template #default>
...
const isHoldingAnalysisMode = computed(() => {
  return route.name === 'HoldingAnalysis' || route.path === '/analysis/holding' || route.query.holding === '1'
})

const pageTitle = computed(() => isHoldingAnalysisMode.value ? '持仓分析' : '单股分析')
const pageDescription = computed(() => (
  isHoldingAnalysisMode.value
    ? '结合成本、仓位、止盈止损和AI分析结果，生成仓位管理参考'
...
const buildHoldingPayload = () => {
  if (!analysisForm.holding.enabled) {
    return null
  }

  const costPrice = Number(analysisForm.holding.costPrice || 0)
  const shares = Number(analysisForm.holding.shares || 0)

  if (costPrice <= 0 || shares <= 0) {
    ElMessage.warning('请填写持仓成本和持股数量')
    return undefined
  }
  ...
  return {
    ...
    shares,
    take_profit_price: analysisForm.holding.takeProfitPrice || undefined,
    stop_loss_price: analysisForm.holding.stopLossPrice || undefined,
    plan: analysisForm.holding.plan
  }
}
...
const submitAnalysis = async () => {
  ...
  const holdingPayload = buildHoldingPayload()
  if (holdingPayload === undefined) {
    return
  }
  ...
  try {
    ...
    const request: SingleAnalysisRequest = {
      ...
      parameters: {
        ...
        ...(holdingPayload ? { holding: holdingPayload } : {})
      }
    }
...
const getHoldingRiskTagType = (riskLevel: string): 'primary' | 'success' | 'warning' | 'info' | 'danger' => {
  if (riskLevel === '高') return 'danger'
  if (riskLevel === '中') return 'warning'
  return 'success'
}

const getProfitClass = (value: number | null | undefined) => {
...
const formatMoney = (value: number | null | undefined) => {
  ...
  return `${value >= 0 ? '+' : ''}${Number(value).toFixed(2)}元`
}
...
const getAnalysisReports = (data: any) => {
  ...
  const reportMappings = [
    ...
    { key: 'holding_analysis', title: '📌 持仓分析', category: '最终决策' },
...
onMounted(async () => {
  ...
  if (isHoldingAnalysisMode.value) {
    analysisForm.holding.enabled = true
  }

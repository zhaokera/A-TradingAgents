...
class DataSourceManager:
    ...
    def _run_async(awaitable):
        """Run an async provider call from sync code, including inside running event loops."""
        import asyncio
        import threading
    ...
    @staticmethod
    def _normalize_data_result(result) -> tuple[str, Optional[str]]:
        """Normalize data source results to (result_text, source_name)."""
    ...
    def _is_success_result(result: Any) -> bool:
        return isinstance(result, str) and bool(result) and "❌" not in result and "错误" not in result
    ...
    def get_stock_data(self, symbol: str, start_date: str = None, end_date: str = None, period: str = "daily") -> str:
        ...
        try:
            ...
            if is_success:
                ...
                # 数据质量异常时也尝试降级到其他数据源
                fallback_result, fallback_source = self._normalize_data_result(
                    self._try_fallback_sources(symbol, start_date, end_date, period)
                )
                if self._is_success_result(fallback_result):
                    display_source = fallback_source or "备用数据源"
                    logger.info(f"✅ [数据来源: {display_source}] 降级成功获取数据: {symbol}")
            ...
            fallback_result, _ = self._normalize_data_result(
                self._try_fallback_sources(symbol, start_date, end_date, period)
            )
            return fallback_result

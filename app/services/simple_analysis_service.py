...
class SimpleAnalysisService:
    ...
    def _resolve_current_price(self, code: Optional[str]) -> Optional[float]:
        ...
        try:
            ...
            if not isinstance(info, dict):
                return None
            for key in ("current_price", "price", "close", "latest_price"):
                value = info.get(key)
                if value is None or value == "":
                    continue
                try:
            ...
            logger.warning(f"⚠️ 获取股票当前价失败: {code} - {e}")
        return None
    ...
    def _run_analysis_sync(
        ...
        try:
            ...
            if request.parameters and request.parameters.holding:
                try:
                    current_price = self._resolve_current_price(request.get_symbol())
                    holding_analysis = build_holding_analysis(
                        request.parameters.holding,
                        formatted_decision,
                        current_price=current_price,
                    )
                    if holding_analysis:
                        holding_report = format_holding_analysis_markdown(holding_analysis)
                        reports["holding_analysis"] = holding_report
                        recommendation = f"{recommendation}\n\n{holding_report}"
                        logger.info("📌 [HOLDING] 已生成持仓分析")
                except Exception as e:
                    logger.warning(f"⚠️ [HOLDING] 生成持仓分析失败: {e}")
